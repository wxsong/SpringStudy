 ...
 * @constructor
 */
function JSONFilter(input, filename, basePath, preparedData) {
    var data;
    data = util.removeComment(input);
    return data;
}
/**
 * 处理smarty模板文件
 ...
 * @constructor
 */
function HTMLFilter(input, filename, basePath, preparedData) {
    var data={};
    logger.trace('Smarty模板路径:' + filename);
    var _path = path.dirname(filename);
    if (fileutil.isExist(path.join(_path, 'data.json'))) {
        try {
            var rawContent = fileutil.getContentText(path.join(_path, 'data.json'), 'utf-8');
            rawContent = util.removeComment(rawContent);
            data = global.eval('(' + rawContent + ")");
        } catch (ex) {
            data = {};
            logger.error('Mock数据有误'+path.join(_path, 'data.json'));
        }
    }
    preparedData && (data = util.extend(data, preparedData));
    var _html = '';
    try {
        var reMarker = new smarty.reMarker();
        reMarker.setBasePath(basePath);
        global.config && (data = util.extend(data, global.config));
...
function RouteHandler(req, res) {
    global.routes.every(function (item) {
        if (req.url.match(item.key)) {
            if (fileutil.isExist(req.url.replace(item.key, item.value))) {
                req.ori_url = req.url;
    ...
    fs.exists(filename, function (exists) {
        ...
        readStream.on('end', function () {
            ...
            if (ext == '.html' || ext == '.htm') {
                ...
                html = bufferHelper.toBuffer().toString('UTF8');
                _output = HTMLFilter(html, filename, req.basePath);
                header['Content-Length'] = Buffer.byteLength(_output, 'utf-8');
            }else if(ext == '.json'){
                html = bufferHelper.toBuffer().toString('UTF8');
                _output =  html;
                header['Content-Length'] = Buffer.byteLength(_output, 'utf-8');
                ...
                console.log(ext);
                _output = bufferHelper.toBuffer();//.toString();
        ...
        /*fs.readFile(filename, 'binary', function (err, file) {
            ...
            if (ext == '.html' || ext == '.htm') {
                ...
                _output = HTMLFilter(file, filename, req.basePath);
                header['Content-Length'] = Buffer.byteLength(_output, 'binary');

            } else if (ext == '.json') {
                //file = iconv.decode(new Buffer(file), 'utf-8');
                file = file.toString('UTF8');
                _output = JSONFilter(file, filename, req.basePath);
                header['Content-Length'] = Buffer.byteLength(_output, 'binary');
